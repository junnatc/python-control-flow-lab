...
def calculate_dog_years():
    dog_age = int(input("Input a dog's age: "))  # Convert input to an integer
    
    if dog_age <= 2:
        dog_years = dog_age * 10  # First 2 years count as 10 dog years each
    else:
        dog_years = 20 + (dog_age - 2) * 7  # First 2 years = 20 dog years, then 7 dog years for each additional year
...
def determine_season():
    ...
    try:
        ...
        if month not in month_to_num:
            print("Invalid month. Please enter a valid month in the format 'Jan', 'Feb', etc.")
            return

        # Get numerical value for the month
        month_num = month_to_num[month]

        # Ensure the day is valid for the month (basic check)
        if day < 1 or day > 31:
            print("Invalid day of the month. Please enter a day between 1 and 31.")
            return
